...
DIARY_SUMMARY_SYSTEM_PROMPT = """당신은 다정한 회고 코치입니다.
사용자가 실제로 쓴 일기 원문만을 근거로, 날짜별 감정과 있었던 일 위주로
짧게 요약하고 주간 총평을 작성합니다.
일기에 없는 사실이나 감정을 지어내지 마세요.
반드시 유효한 JSON만 출력하세요. 마크다운 코드블록 없이. 한국어로 응답하세요.
"""


def _build_diary_summary_prompt(diary_items: list[dict]) -> str:
    entry_lines = []
    ...
    return f"""아래는 이번 주에 실제로 작성된 일기 원문입니다. 감정과 있었던 일 위주로 분석하세요.

{entries_section}

## 작성 지침
- 날짜별로 1~2문장씩, 그날의 감정과 있었던 일 위주로 요약하세요 (일기 본문이 있는 날짜만).
- 일기에 등장하지 않는 사건이나 감정을 지어내지 마세요.
- 마지막에 한 주 전체의 감정 흐름과 눈에 띄는 사건을 2~3문장으로 총평하세요.

## 출력 JSON 형식
{{
  "entries": [
    {{"date": "YYYY-MM-DD", "day_name": "월", "text": "그날의 감정·사건 요약 1~2문장"}}
  ],
  "weekly_summary": "주간 감정·사건 총평 2~3문장"
}}"""


def summarize_diary_entries(diary_items: list[dict]) -> dict | None:
    """실제 일기 기록이 있을 때, 감정/사건 위주 요약을 생성."""
    if not diary_items or not any(item.get('content') for item in diary_items):
        return None

    print('[일기 요약] 감정/사건 요약 생성 중...')
    prompt = _build_diary_summary_prompt(diary_items)

    message = client.messages.create(
        model='claude-sonnet-4-6',
        max_tokens=3000,
        system=DIARY_SUMMARY_SYSTEM_PROMPT,
        messages=[{'role': 'user', 'content': prompt}],
    )
